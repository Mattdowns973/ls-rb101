...
output:
-an array list of the digits, separated by commas
...
-digits should remain in the same order as they were in the original integer
...
Your Mental Model: The method takes on positive integer and splits up the digits and puts them into
an array separated by commas, and leaves them in the same order as in the integer.
...
def digit_list(int)
  string = int.to_s
  list = string.split(//).map! { |x| x.to_i }
end
