...
input:
-given an array with various elements (unknown)

output:
-hash with key of each uniqueoriginal array element and value of # 
of occurrences in array
...
1. Will the array always include only strings?

2. How to handle exceptions or erros in input?
...
Your Mental Model: A method that takes a given array of various elements and returns a hash that
lists each element as a key and its number of occurrences as the value
...
-Output will be a hash with key value pairs matching the element and it's occurrences
...
-iterate over array setting each element equal to a key in a hash, incrementing counter by 1
-if the same element appears again, it will incrememnt counter again
-output a hash with the resulting keys and values
