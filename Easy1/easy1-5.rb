...
output:
one string with the words reversed
...
1. Do there have to be multiple words?

2. Should numbers be treated the same if they are input with/without spaces?
...
Your Mental Model:
The method will take a string, split it into an array with words, reverse the index values, and then
put them back into a string where the words are unchanged, but in reverse order.
...
-Take the given string and split it into an array of spaces and separate word elements.
-Reverse the index values of each element and make the change permanent.
-Convert the reversed array into a string and set equal to a new variable
...
def reverse_sentence(string_input)
  working_array = []
  working_array = string_input.split
  working_array.reverse!
  puts working_array.join(" ")
end
