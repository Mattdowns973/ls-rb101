...
input:
-one integer value (positive, negative, or zero)
...
Problem Domain:
absolute value - distance from zero for negative numbers
...
Your Mental Model: Program takes any integer below, above, or equal to zero and checks to see if 
the absolute value is odd before returning true or false.
...
Your Edge Cases:
Edge cases would be 0 and then both 1 and -1.
...
Input will integer.  Output will be string of boolean value.
...
-Input one integer argument with method
-Save absolute valie of integer input into a new variable, abs_value
-Return true if abs_value divided by 2 is not equal to zero.
-Else Return false if abs_value divided by 2 is equal to zero
